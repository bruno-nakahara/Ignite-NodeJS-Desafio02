...
function checksExistsUserAccount(request, response, next) {
  ...
  return response.status(404).json({ error: "Username doest exists" });
}
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  return response
    .status(403)
    .json({ error: "Username cant create more todos" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (validate(id)) {
    ...
    if (todofiltered) {
      request.user = userfiltered;
      request.todo = todofiltered;
